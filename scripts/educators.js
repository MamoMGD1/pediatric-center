...
function displayAssistantCards() {
    ...
    JSON.parse(localStorage.getItem("storedUsers")).forEach((user) => {
        if(user.userType === "instructor"){
            ...
            card.innerHTML = `
                <i class="fa-solid fa-user-doctor" style="margin: 0; margin-top:5px;" id="userIcon"></i>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    setTimeout(() => {
        const doctorName = decodeURIComponent(window.location.hash.substring(1));
        if (doctorName) {
...
function promptAssistantDetails() {
    ...
    return {
        ...
        password: "teach123" // Assign a default password for new assistant
    };
}
